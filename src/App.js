...
const App = () => {
  ...
  useEffect(() => {
    ...
    const FetchQuestionList = async () => {
      await axios.get('http://localhost:3333/question_list')
        .then((res) => {
    ...
    const FetchOptions = async () => {
      await axios.get('  http://localhost:3333/options_list')
        .then((res) => {
  ...
  //     if (selectedOption == options[i]?.correct_answer_to_question) {
  //       alert('right');
  //     } else {
  //       alert('wrong');
  //     }
    ...
    <div>
        ...
        optionsQ1?.map((item, index) => {
          return (
            <div onChange={onChangeValue}>
              <input type='radio' value={item.correct_answer_to_question} name='hh' />
              {item.title}
            </div>
          );
        })
      ...
      </h3>
      {
        ...
        optionsQ2?.map((item, index) => {
          return (
            <div onChange={onChangeValue}>
              <input type='radio' value={item.correct_answer_to_question} name='test' />
              {item.title}
            </div>
          );
        })
